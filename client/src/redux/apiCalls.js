...
export const getCart = async()=> {
  try {
    const res = await axios.get("http://localhost:3030/api/carts/find/"+store.getState().user.currentUser._id,{headers: {token: `Bearer ${store.getState().user.currentUser.accessToken}`}})
    if(res.data){
      console.log(res.data);
      return res.data;
    }
...
export const deleteCart = async(id)=>{
  try {
    await axios.delete("http://localhost:3030/api/carts/"+id,{headers: {token: `Bearer ${store.getState().user.currentUser.accessToken}`}})
  } catch (error) {
    console.log(error);
  }
...
export const addOrder = async(order) => {
  try {
    await axios.post("http://localhost:3030/api/orders",order,{headers: {token: `Bearer ${store.getState().user.currentUser.accessToken}`}})
  } catch (err) {
    console.log(err);
  }
}
